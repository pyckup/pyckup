...
def start_service(
    name, conversation_config_base64, sip_credentials_base64, num_devices, openai_key
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Start Pyckup instance as a systemd service."
    )
    parser.add_argument("name", type=str, help="Name of the service")
    parser.add_argument(
        "conversation_config_base64",
        type=str,
        help="Base64 encoded conversation configuration",
    )
    parser.add_argument(
        "sip_credentials_base64", type=str, help="Base64 encoded SIP credentials"
    )
    parser.add_argument(
        "num_devices", type=int, help="Number of possible simultaneous devices"
    )
    ...
    start_service(
        args.name,
        args.conversation_config_base64,
        args.sip_credentials_base64,
        args.num_devices,
        args.openai_key,
    )
